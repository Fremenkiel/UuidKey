...
internal static class Blake2IvBuilder
{
	...
	public static ulong[] ConfigB(Blake2BConfig config, Blake2BTreeConfig? treeConfig)
	{
		...
		if (config.OutputSizeInBytes <= 0 | config.OutputSizeInBytes > 64)
			throw new ArgumentOutOfRangeException(nameof(config), "Output size must be between 0 and 64");
		...
		if (config.Key != null)
		{
			if (config.Key.Length > 64)
				throw new ArgumentException("Key too long", nameof(config));
		...
		// Inner length
		if (!isSequential && treeConfig.IntermediateHashSize is <= 0 or > 64)
			throw new ArgumentOutOfRangeException(nameof(treeConfig), "Invalid intermediate hash size");
